...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            pos.append((x, y))
    variableDomainsDict[PAC] = pos.copy()
    variableDomainsDict[GHOST0] = pos.copy()
    variableDomainsDict[GHOST1] = pos.copy()
    # Observations here are non-negative, equal to Manhattan distances of Pacman to ghosts ± noise.
    dist = abs(0 - X_RANGE) + abs(0 - Y_RANGE) + MAX_NOISE - 1
    observ = list(range(dist))
    variableDomainsDict[OBS0] = observ.copy()
    variableDomainsDict[OBS1] = observ.copy()
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict,
        ...
        "*** YOUR CODE HERE ***"
        factorsToJoin = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for elimVar in eliminationOrder:
            factorsToJoin, joinedFactor = joinFactorsByVariable(factorsToJoin, elimVar)

            if len(joinedFactor.unconditionedVariables()) <= 1:
                continue

            eliminatedFactor = eliminate(joinedFactor, elimVar)
            factorsToJoin.append(eliminatedFactor)

        resultFactor = joinFactors(factorsToJoin)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            if position == jailPosition:
                ...
                prob_obs_given_pos = self.getObservationProb(observation, pacmanPosition, position, jailPosition)
                self.beliefs[position] *= prob_obs_given_pos
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newB = DiscreteDistribution()
        for oldP in self.allPositions:
            newPDist = self.getPositionDistribution(gameState, oldP)

            for newP, prob in newPDist.items():
                newB[newP] += self.beliefs[oldP] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        P_per_pos = self.numParticles // numPos
        remainingP = self.numParticles % numPos

        for pos in self.legalPositions:
            self.particles.extend([pos] * P_per_pos)
        for i in range(remainingP):
            self.particles[i] = self.legalPositions[i]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        observ = DiscreteDistribution()
        pos = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()

        for p in self.particles:
            prob = self.getObservationProb(observation, pos, p, jailPosition)
            observ[p] += prob

        if observ.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [observ.sample() for i in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
