    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for action in legal:
            ...
            for distribution in livingGhostPositionDistributions:
                likelyPos = distribution.argMax()
                prob = distribution[likelyPos]
                dist = self.distancer.getDistance(successor_position, likelyPos)
                expectedDist += prob * dist

            if expectedDist < minDist:
                minDist = expectedDist
                bestA = action
